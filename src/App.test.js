...
import { render, waitFor, getAllByTestId, fireEvent } from '@testing-library/react'
import App from './App'
...
test('render app with data loaded', async () => {
    ...
    await waitFor(() => {expect(getByText(/the vanishing of will byers/i)).toBeInTheDocument()})
    await waitFor(() => {expect(getByText(/the flea and the acrobat/i)).toBeInTheDocument()})

    await userEvent.click(getAllByText(/season 1/i)[0])
    await fireEvent.click(getByText(/season 2/i))

    await waitFor(() => {expect(getByText(/trick or treat, freak/i)).toBeInTheDocument()})
    await waitFor(() => {expect(getByText(/will the wise/i)).toBeInTheDocument()})
